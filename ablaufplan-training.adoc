...
** Documemnt Type: XML (JSON)
** Documents: <delete><query>*:*</query></delete> ({'delete': {'query': '*:*'}})

* mittels curl
[source,bash]
----
curl http://localhost:8983/solr/cars/update -H "Content-type: text/xml" --data-binary '<delete><query>*:*</query></delete>'
curl http://localhost:8983/solr/cars/update -H "Content-type: text/xml" --data-binary '<commit />'
----
